...
export function isValid() {
  ...
  let variableSubcktList = [
    convertToLowerCase(subcktName.value.trim()),
    convertToLowerCase(inv_instance1.value.trim()),
    convertToLowerCase(inv_instance2.value.trim()),
    convertToLowerCase(mux_instance_name.value.trim()),
    "vdd",
    "gnd",
    "wmin",
    "lmin",
  ...
  if (
    mux_instance_name.value.trim()[0] != "x" &&
    mux_instance_name.value.trim()[0] != "X"
  ) {
    let msg =
      "When instantiating a sub circuit, the name of the instance must always start with 'x' or 'X'";
...
export function printObsTable() {
  ...
  let subcktIn2 = document.getElementById("subckt-in2-name");
  let subcktOut = document.getElementById("subckt-out-name");
  let inv_in1 = convertToLowerCase(
    document.getElementById("inv-call1-input").value.trim()
  );
  let inv_out1 = convertToLowerCase(
    document.getElementById("inv-call1-output").value.trim()
  );
  let inv_in2 = convertToLowerCase(
    document.getElementById("inv-call2-input").value.trim()
  );
  let inv_out2 = convertToLowerCase(
    document.getElementById("inv-call2-output").value.trim()
  );
  let mux_a = convertToLowerCase(
    document.getElementById("mux-a-input").value.trim()
  );
  let mux_b = convertToLowerCase(
    document.getElementById("mux-b-input").value.trim()
  );
  let mux_s = convertToLowerCase(
    document.getElementById("mux-s-input").value.trim()
  );
  let mux_out = convertToLowerCase(
    document.getElementById("mux-output").value.trim()
  );
  let gateCallIn1 = document.getElementById("gate-call-input1");
  let gateCallIn2 = document.getElementById("gate-call-input2");
  let gateCallOut = document.getElementById("gate-call-output");
  ...
  if (
    ...
    convertToLowerCase(gateCallIn1.value.trim()) !== "clk"
  ) {
    correct = false;
  }
  if (
    convertToLowerCase(gateCallIn2.value.trim()) !== "a" &&
    convertToLowerCase(gateCallIn2.value.trim()) !== "clk"
  ) {
    correct = false;
  }
  if (
